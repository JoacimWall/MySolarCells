...
public class HomeAssistentInverterService : IInverterServiceInterface
{
    ...
    public async Task<Result<List<PickerItem>>> GetPickerOne()
    {
        ...
        if (!result.WasSuccessful)
            return new Result<List<PickerItem>>(result.ErrorMessage);
        else
        {
            ...
            {
                returnlist.Add(new PickerItem { ItemValue = fakeId, ItemTitle = item.entity_id.ToString() });
                fakeId++;
            }
    ...
    public async Task<Result<GetInverterResponse>> GetInverter(PickerItem pickerItem)
    {
        ...
        return new Result<GetInverterResponse>( new GetInverterResponse { InverterId = pickerItem.ItemTitle, Name = pickerItem.ItemTitle });
    }
