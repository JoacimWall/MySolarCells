    ...
    public async Task<bool> SyncProductionOwnUse(DateTime start, IProgress<int> progress, int progressStartNr)
    {
        ...
        try
        {
            ...
            while (start < end)
            {
                ...
                {
                    processingDateFrom = new DateTime(start.Year, start.Month, start.Day).ToString("yyyy-MM-dd") + " 00:00:00";
                    processingDateTo = new DateTime(end.Year, end.Month, end.Day).ToString("yyyy-MM-dd") + " 23:59:59";
                    nextStart = end;
                }
                ...
                while (startDays < nextStart)
                {
                    ...
                    {
                        processingDateFrom = new DateTime(startDays.Year, startDays.Month, startDays.Day).ToString("yyyy-MM-dd") + " 00:00:00";
                        processingDateTo = new DateTime(startDays.Year, startDays.Month, startDays.Day).ToString("yyyy-MM-dd") + " 00:00:00";
                ...
                for (int i = 0; i < producedI; i++)
                {
                    ...
                    if (energyExist != null)
                    {
                        ...
                        energyExist.BatteryCharge = sumes[i].batteryCharge > 0 ? Convert.ToDouble(sumes[i].batteryCharge) / 1000 : 0;
                        energyExist.BatteryUsed = sumes[i].batteryOutput > 0 ? Convert.ToDouble(sumes[i].batteryOutput) / 1000 : 0;
